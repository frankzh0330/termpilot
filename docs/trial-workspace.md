...
The current `/trial start` command works even if `enabled` is false. The flag is reserved for later automatic trial-mode routing.
...
This keeps the design ready for a future standalone runtime service.
...
- After `/trial apply`, accepted changes appear in the source project.
- `/trial discard` removes the trial copy.

## Current Limitations

- Trial mode is manual; automatic risk-based routing is not implemented yet.
- Apply currently copies changed files back directly; source-drift detection and patch backups are future work.
...
- Cleanup/TTL is planned but not implemented yet.
